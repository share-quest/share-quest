...
export default function App() {
  ...
  // 権限管理: 'guest', 'reader' (ログイン済), 'writer' (ライター), 'editor' (編集長)
  ...
  const getFontSizeClass = () => {
    return { small: "text-sm", medium: "text-base", large: "text-lg" }[
      fontSize as keyof typeof fontSize_map
    ];
  };
  const fontSize_map = { small: "text-sm", medium: "text-base", large: "text-lg" };
            ...
            {myArticles.map((article) => (
              ...
              >
                ...
                <div className="flex gap-2 mt-3">
                  <button
                    onClick={() => showToast("記事編集(モック: 画像貼付/タグ付/文字色変更など)")}
